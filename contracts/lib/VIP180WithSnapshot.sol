...
import {VIP180} from '../lib/VIP180.sol';
import {ITransferHook} from '../interfaces/ITransferHook.sol';

/**
 * @title VIP180WithSnapshot
 * @notice VIP180 including snapshots of balances on transfer-related actions
 * @author VeBank
 **/
contract VIP180WithSnapshot is VIP180 {
    ...
    /// @dev reference to the VeBank governance contract to call (if initialized) on _beforeTokenTransfer
    /// !!! IMPORTANT The VeBank governance is considered a trustable contract, being its responsibility
    /// to control all potential reentrancies by calling back the this contract
    ITransferHook public _vebankGovernance;

    event SnapshotDone(address owner, uint128 oldValue, uint128 newValue);

    constructor(string memory name, string memory symbol, uint8 decimals) public VIP180(name, symbol, decimals) {}

    function _setVeBankGovernance(ITransferHook vebankGovernance) internal virtual {
        _vebankGovernance = vebankGovernance;
    }
    ...
    */
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal override {
        ...
        // caching the VEBANK governance address to avoid multiple state loads
        ITransferHook vebankGovernance = _vebankGovernance;
        if (vebankGovernance != ITransferHook(0)) {
            vebankGovernance.onTransfer(from, to, amount);
        }
    }
}